...
const uploadSingleImage = async (base64Image: string, imageIndex: number): Promise<boolean> => {
    ...
    const uploadSingleImage = async (base64Image: string, imageIndex: number): Promise<boolean> => {
        ...
        // STRATEGY 1: Direct Input Injection (Try everywhere first)
        ...
        return injected; // Return true if we managed to inject at all
    };
    ...
    export const runTwoStagePipeline = async (config: PipelineConfig): Promise<{
        ...
        try {
            ...
            await fillPromptAndGenerate(config.imagePrompt); // This contains "create a prompt"
            await delay(2000);
